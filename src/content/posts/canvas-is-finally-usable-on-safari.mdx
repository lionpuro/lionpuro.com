...
![Canvas area exceeds the maximum limit (width * height > 16777216).](/images/posts/safari-canvas-error.png)

That's probably more than enough pixels for most use cases, but we found it really... _limiting_.
...
Next, I ran some tests using [canvas-size](https://www.npmjs.com/package/canvas-size), which showed that the new limit is **8192x8192** with a total area of
**67,108,864** pixels. Seeing that nothing had changed for my old iPad running
iOS 17.7.5, it seemed like they most likely changed it in iOS 18.0, but I had to
be confirm that another way, as my phone was running on a newer minor version.

So, I signed up for [BrowserStack](https://www.browserstack.com) and ran the same tests on their
